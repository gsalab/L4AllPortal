/*Script (oggetto) che costruisce il canvas Mosaico */


//Classe, con gli attrubuti che servono per gestire il suo stato
mosaicView = function(containerElmt,expression) {
    ...
    SettingsUtilities._internalCollectSettings(this._id_div, mosaicView._settingSpecs, this._settings); //carica i valori delle variabili (vedi script /utils/)
...
*/
mosaicView._settingSpecs = {
    //parametri per lo schema del canvas
    "label": {type: "text", defaultValue: "Mosaic View"}, //etichetta del canvas (quella che verrà mostrata nel menu a tendina della scelta del canvas)
    "raw": { type: "int", defaultValue: null }, //numero di righe del mosaico
    "col": { type: "int", defaultValue: 18 }, //numero di colonne del mosaico
...
mosaicView.prototype.sortBy = function(){
    ret="";
...
mosaicView.prototype.fieldParams = function(){
  var ret = [];
  ret.push('id');
  ret.push('label');
...
mosaicView.prototype.buildCanvas = function(data){
  ...
  /*VIA CON LA COSTRUZIONE DEL MOSAICO
  ...
  for(var d=0; d<data.length; d++){
    ...
    html = html+ '<div idObj="'+data[d].id+'" class="mosaicCell on" title="'+eval('data[d].'+this._settings.etichettaIcone).replace(/"/g, '&quot;')+'" colorCode="'+ C +'"> <img src="'+ S +'"/> </div>'; //i css le faranno flottare in modo tale che vadano a capo da sole
    /* D.T. 10/01/2013 BEGIN */
...
mosaicView.prototype.displayCanvas = function(data){
    var self=this;
    ...
    if(this._highLabels.length > 1){
     ...
     if($('#refraso').get()[0]!=null) $('#refraso').before(this._htmlCanvas); 
     else $('#view').append(this._htmlCanvas);
     for(i in this._mapsColors[this._currentHighlight]){ //sistema colori
            var sel = '#view .mosaicCell.on[colorcode="'+this._mapsColors[this._currentHighlight][i]+'"]';  
      ...
      $('#view .mosaicCell').css('width',val+'px');
      $('#view .mosaicCell').css('height',val+'px');
      
      //evento per l'apertura delle lens
      //if(self._settings.lenstype!=null) 
      	$('#view .mosaicCell').click(function(){ sendLensRequest($(this).attr('idobj'), Application.getContestoAttuale().getItemType())});
}
...
*/
mosaicView.prototype.releaseCanvas = function(){
   if(this._currentHighlight != 0){
    ...
    $('#view .viewContainer').remove();
    $('#view .legendaShape').remove();
}

/*Funzione chiamata per fare l'update del canvas: molto + leggera di quella iniziale! */
mosaicView.prototype.updateCanvas = function(data){
  var self= this;
  $('#view .mosaicCell').removeClass('on'); //spegni tutto
  $('#view .mosaicCell').unbind(); //togli gli eventi delle lens
  $('#view .mosaicCell').css('background-color','inherit'); //sfondo neutro
  ...
  for(var d=0; d<data.length; d++){ //riaccendi solo quelle della lista!
    var sel = '#view .mosaicCell[idobj="'+data[d].id+'"]';
    $(sel).addClass('on');
    $(sel).css('background-color',$(sel).attr('colorCode'));
    $(sel).click(function(){ 
       sendLensRequest($(this).attr('idobj'), Application.getContestoAttuale().getItemType());
    });
    /* D.T. 10/01/2013 BEGIN */
...
mosaicView.prototype.changeColorCode = function(index){
  if(this._currentHighlight != index){
        this._currentHighlight = index;
        var nodes = $('#view .viewContainer .mosaicCell').get();
